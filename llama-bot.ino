...
Song takeOnMe = {
  140,
  takeOnMeMelody,
  sizeof(takeOnMeMelody)
};

int babyElephantMelody[] = {
...
#define SERVO_TIMING_MS 30
#define SERVO_MAX 100
#define SERVO_MIN 45
#define SERVO_NEGATIVE_STEP -2
#define SERVO_POSITIVE_STEP 7
...
void setup() {
  ...
  Serial.print("song size:");
  Serial.print(sizeof(songs));
  Serial.print("\n");
...
void BuzzerControlLoop() {
  ...
  int melodySize = songs[songIndex].melodySize;
  ...
  if (buzzerPosition == notes * 2) {
    ...
    if (!isPlayingNote) {
      ...
      tone(buzzer, melody[buzzerPosition]);
      isPlayingNote = true;
    }
    if (isPlayingNote && buzzerChrono.hasPassed(noteDuration)) {
      ...
      isPlayingNote = false;
      noTone(buzzer);
      buzzerPosition = buzzerPosition + 2;
    }
  }
}
